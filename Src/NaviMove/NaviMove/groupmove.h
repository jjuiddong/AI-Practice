...
namespace ai
{
	...
	{
	public:
		cGroupMove(ai::iActorInterface<T> *agent, const vector<Vector3> &path
		...
		virtual bool ActionExecute(const float deltaSeconds) override
		{
			...
			if (isMoveCollision)
			{
				...
				else
				{
					...
					m_collisionInterval = 0.3f;
					m_dir = newDir;
					pos = collSphere.GetPos() + toMe * (collSphere.GetRadius() + bsphere.GetRadius());
					//dbg::Log("collision \n");
				}
			}
